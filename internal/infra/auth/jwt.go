...
func (m *Manager) NewClientToken(subject, documentType, documentNumber string) (string, time.Time, error) {
	if len(m.secret) == 0 {
		return "", time.Time{}, fmt.Errorf("%w: JWT secret not configured", ErrUnauthorized)
	}
	if subject == "" {
	...
	if documentNumber == "" {
		return "", time.Time{}, fmt.Errorf("document number is required")
	}
